...
Most legged robots are designed the same way: fix the mechanical
design first, then build a controller to work with them. This
project explores what happens when you flip that i.e treating a
robot's physical proportions, motion trajectory, and control policy
as coupled variables optimized together, rather than in the
...
as the free design parameters. Two different controllers were built
to drive the jump - a real-time Quadratic Programming (QP)
task-space controller, and a trajectory optimizer (CasADi + IPOPT)
...
{{< figure src="flow.png" alt="Flowchart" caption="Flowchart showing the entire pipeline" >}}


## Key Findings

- Both controllers converged to nearly identical optimal
  morphologies independently, suggesting that the task itself and not the
...
- The trajectory optimizer consistently beat the QP controller in
  ...
  distribution across the whole stance phase rather than reacting
  greedily.
- Each morphology performed best paired with the controller it was
  co-designed with giving direct evidence that link geometry, trajectory,
  and control aren't separable design choices.
