...
## Overview
Designed and simulated the attitude control system for a student-built CubeSat in a 500 km Sun-Synchronous Orbit: detumbling with magnetorquers, precision pointing with reaction wheels, and ground-station tracking maneuvers. The work covered control design, dynamics modeling, and trajectory generation.

<img src="simu.png"  class="full-bleed-image">

## 1. Detumbling — B-dot Algorithm
- Generated the SSO orbit (500 km, 97.5° inclination) and modeled the local magnetic field with the IGRF model.
- Built a Simulink model implementing the B-dot control law for a 31.25 cm cube satellite (MOI: [3, 3, 10] kg·m²) actuated by magnetorquers (max moment 5 A·m²).
...
- The gap between the two shows how much the initial spin axis affects detumbling time.
...
- Found that the original reference trajectory was continuous but not differentiable at maneuver transitions, which caused discontinuous angular velocity commands and large torque spikes.
- Redesigned the transition function for first-order smoothness between limb-pointing and GS-tracking segments.
- **Result:** cleaner controller output, fewer oscillations, and better tracking accuracy, confirmed by comparing angle and angular-velocity profiles before and after the change.
